  ...
  return (
    <>
      <button className="relative" onClick={handleFavourite} disabled={loading}>
        {loading ? (
          <div className="border-b-2 border-primary rounded-full animate-spin w-12 h-12" />
        ) : (
          <Tooltip
          ...
          >
            ...
            />
            <span className={`absolute top-0 right-0 w-5 h-5 rounded-full flex items-center justify-center text-[0.7rem] p-1 ${isLiked ? "bg-white text-primary shadow-lg" : "bg-[#9932cc] text-white"}`}>
